...
import com.example.familysong.B.B0.B0_1Activity;
import com.example.familysong.R;

public class F0_1Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState)//生命周期
    {
        ...
        setContentView(R.layout.activity_f01);
        videoView=findViewById(R.id.videoplay);
        icon=findViewById(R.id.icon);
        //创建视频资源的文件路径
        String url="android.resource://"+getPackageName()+"/"+R.raw.f;
        Uri uri= Uri.parse(url);
        videoView.setVideoURI(uri);//将要播放的视频路径指定给控件
        //创建一个视频的控制器
        mediaController=new MediaController(F0_1Activity.this);
        //J将控制器交给视频播放的控件
